...
def generate_tweet_service(topic, db):
    ...
    # Save the full dictionary (content, hashtags, full_tweet)
...
def post_tweet(id, db):
    ...
    # Extract full tweet content from stored dict
    full_tweet = tweet.content.get("full_tweet") if isinstance(tweet.content, dict) else str(tweet.content)
...
def update_tweet(id, topic, content, db):
    ...
    if content:
        # If content is expected to be a string update only 'full_tweet'
        if isinstance(tweet.content, dict):
            tweet.content["full_tweet"] = content
        else:
            tweet.content = {"full_tweet": content}
